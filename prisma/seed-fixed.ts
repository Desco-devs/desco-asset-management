...
async function main() {
  ...
  // Create locations - fixed table name from 'location' to match schema
  await Promise.all([
    prisma.location.upsert({
      ...
      create: {
        ...
        address: "TIWI",
        created_by: null, // No user reference for now
      },
    }),
    prisma.location.upsert({
  ...
  // Create clients - fixed table name from 'client' to match schema
  await Promise.all([
  ...
  // Create projects - fixed table name from 'project' to match schema
  await Promise.all([
  ...
  // Create equipment - fixed schema compliance
  await Promise.all([
  ...
  // Create vehicles - fixed schema compliance
  await Promise.all([
  ...
  // Create maintenance reports - fixed user references
  await Promise.all([
    prisma.maintenance_equipment_report.upsert({
      ...
      create: {
        ...
        location_id: "abd30557-34ef-45b2-98e9-2906651a1cc2",
        reported_by: null, // No user reference for now
        repaired_by: null, // No user reference for now  
        issue_description: "-",
        remarks: "-",
        inspection_details: "-",
        action_taken: "-",
