...
export default function VehiclesListModern() {
  ...
  const searchQuery = useVehiclesStore(selectSearchQuery);
  ...
  const getFilteredVehicles = useVehiclesStore(
    (state) => state.getFilteredVehicles
  );
  // const getSortedVehicles = useVehiclesStore(
  //   (state) => state.getSortedVehicles
  // );
  // const getPaginatedVehicles = useVehiclesStore(
  //   (state) => state.getPaginatedVehicles
  // );
  // const getTotalPages = useVehiclesStore((state) => state.getTotalPages);
  const getEffectiveItemsPerPage = useVehiclesStore(
  ...
  const sortBy = useVehiclesStore(selectSortBy);
  const sortOrder = useVehiclesStore(selectSortOrder);
        ...
        <div className="relative">
          ...
          <Input
            placeholder={
              isMobile
                ? "Search vehicles..."
                : "Search vehicles by brand, model, plate, owner, project, client, location..."
            }
            value={searchQuery}
          ...
          <Select
            ...
            onValueChange={(value) => {
              ...
              if (value === "created_at") {
                ...
                setSortBy(field as "created_at" | "brand" | "status" | "expiry_date" | "model" | "plate_number" | "owner" | "");
                setSortOrder("desc");
              } else {
                // Default asc handling
                setSortBy(value as "created_at" | "brand" | "status" | "expiry_date" | "model" | "plate_number" | "owner" | "");
                setSortOrder("asc");
              }
            }}
          >
              ...
              <Badge variant="secondary" className="gap-1 pr-1">
                ...
                {(() => {
                  ...
                  if (sortBy === "plate_number")
                    return sortOrder === "asc" ? "Plate A-Z" : "Plate Z-A";
                  if (sortBy === "owner")
                ...
                <div className="pb-3 border-t text-xs text-muted-foreground space-y-1">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span
                      ...
                      title={vehicle.project?.name || "Unassigned"}
                    >
                  ...
                  <div className="flex justify-between">
                    ...
                    <span
                      className="font-medium truncate ml-2"
                      title={vehicle.project?.client?.name || "No client"}
                    >
                  ...
                  <div className="flex justify-between">
                    ...
                    >
                      {vehicle.project?.client?.location?.address?.split(
                        ","
                      )[0] || "No location"}
                    </span>
                  </div>
