...
export default function EditMaintenanceReportModal() {
  const selectedMaintenanceReport = useVehiclesStore(
    (state) => state.selectedMaintenanceReport
  );
  ...
  useEffect(() => {
    if (selectedMaintenanceReport) {
      setFormData({
        ...
        location_id:
          selectedMaintenanceReport.location_id ||
          selectedMaintenanceReport.location?.id ||
          "",
        parts_replaced:
          selectedMaintenanceReport.parts_replaced?.length > 0
            ? selectedMaintenanceReport.parts_replaced
            : [""],
        attachment_urls:
          selectedMaintenanceReport.attachment_urls?.length > 0
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const cleanData = {
      ...
      ...formData,
      parts_replaced: formData.parts_replaced.filter(
        (part) => part.trim() !== ""
      ),
      attachment_urls: formData.attachment_urls.filter(
        (url) => url.trim() !== ""
      ),
