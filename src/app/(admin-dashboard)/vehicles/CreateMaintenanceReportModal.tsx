...
import { useVehiclesWithReferenceData, useCreateMaintenanceReport } from "@/hooks/useVehiclesQuery";
import { toast } from "sonner";

interface CreateMaintenanceReportModalProps {
  vehicleId: string;
}

export default function CreateMaintenanceReportModal({ vehicleId }: CreateMaintenanceReportModalProps) {
  const isOpen = useVehiclesStore(state => state.isMaintenanceModalOpen);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createMaintenanceReportMutation.mutateAsync(cleanData);
      handleClose();
    } catch (error) {
