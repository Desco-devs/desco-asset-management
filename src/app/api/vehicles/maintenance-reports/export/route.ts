...
export const GET = withResourcePermission('maintenance_reports', 'view', async (request: NextRequest, user: AuthenticatedUser) => {
  try {
    ...
    const reportType = searchParams.get('reportType') || 'detailed'; // summary or detailed

    // Build query filters
    const where: any = {};
    if (vehicleId && vehicleId !== 'ALL_VEHICLES') where.vehicle_id = vehicleId;
    if (status && status !== 'ALL_STATUS') where.status = status;
    if (priority && priority !== 'ALL_PRIORITIES') where.priority = priority;
    if (startDate || endDate) {
      where.date_reported = {};
      if (startDate) where.date_reported.gte = new Date(startDate);
    ...
    if (format === 'pdf') {
      const pdfBuffer = await generatePDF(reports, reportType, user);
      
      return new NextResponse(pdfBuffer, {
        headers: {
          'Content-Type': 'application/pdf',
          'Content-Disposition': `attachment; filename="vehicle-maintenance-reports-${new Date().toISOString().split('T')[0]}.pdf"`
        }
      });
    } else if (format === 'excel') {
      const excelBuffer = await generateExcel(reports, reportType, user);
      
      return new NextResponse(excelBuffer, {
        headers: {
          'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
          'Content-Disposition': `attachment; filename="vehicle-maintenance-reports-${new Date().toISOString().split('T')[0]}.xlsx"`
        }
      });
    } else {
    ...
    console.error('Error exporting vehicle maintenance reports:', error);
...
// Generate PDF report
async function generatePDF(reports: any[], reportType: string, user: AuthenticatedUser): Promise<Buffer> {
  const doc = new jsPDF();
  
  // Header
  doc.setFontSize(20);
  doc.text('Vehicle Maintenance Reports', 105, 15, { align: 'center' });
  
  doc.setFontSize(12);
  doc.text(`Generated on: ${new Date().toLocaleDateString()}`, 105, 25, { align: 'center' });
  doc.text(`Generated by: ${user.full_name || user.username}`, 105, 32, { align: 'center' });
  ...
  if (reportType === 'summary') {
    ...
    autoTable(doc, {
      head: [['Vehicle', 'Plate #', 'Status', 'Priority', 'Reported', 'Repaired', 'Issue']],
      body: summaryData,
      startY: yPosition,
    ...
    reports.forEach((report, index) => {
      ...
      doc.text(`Report #${index + 1}: ${report.vehicle.brand} ${report.vehicle.model}`, 20, yPosition);
      yPosition += 10;

      // Vehicle info
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      doc.text(`Plate Number: ${report.vehicle.plate_number}`, 20, yPosition);
      doc.text(`Project: ${report.vehicle.project.name}`, 120, yPosition);
      yPosition += 7;
      
      doc.text(`Client: ${report.vehicle.project.client.name}`, 20, yPosition);
...
async function generateExcel(reports: any[], reportType: string, user: AuthenticatedUser): Promise<Buffer> {
  const workbook = XLSX.utils.book_new();
  
  if (reportType === 'summary') {
    // Summary worksheet
    const summaryData = reports.map(report => ({
      'Vehicle': `${report.vehicle.brand} ${report.vehicle.model}`,
      'Plate Number': report.vehicle.plate_number,
      'Project': report.vehicle.project.name,
      'Client': report.vehicle.project.client.name,
  ...
  const metadataSheet = XLSX.utils.json_to_sheet([
    {
      ...
      'Generated By': user.full_name || user.username,
