...
/**
 * PATCH /api/equipments/[uid]/images
 * Updates equipment images with file upload support
 */
  ...
  const handler = withResourcePermission('equipment', 'update', async (req: NextRequest, _user: AuthenticatedUser) => {
    try {
      ...
      const imageConfigs = [
        ...
        {
          formKey: 'thirdpartyInspectionImage',
          keepKey: 'keepExistingThirdpartyInspection',
          existingUrl: existing.thirdparty_inspection_image,
          prefix: 'thirdparty_inspection',
          field: 'thirdparty_inspection_image' as const,
          tag: '3rd-party inspection image',
        },
        {
          formKey: 'pgpcInspectionImage',
          keepKey: 'keepExistingPgpcInspection',
          existingUrl: existing.pgpc_inspection_image,
          prefix: 'pgpc_inspection',
          field: 'pgpc_inspection_image' as const,
          tag: 'PGPC inspection image',
        },
      ]
      ...
      console.error('Equipment images update error:', error)
      return NextResponse.json(
        { 
